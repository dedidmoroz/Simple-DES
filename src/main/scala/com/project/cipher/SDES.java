...
/**
 * Created by speedfire on 06.12.15.
 */
...
public class SDES
{
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            pt = Print.addZeroBits(Integer.toString('?',2));
    ...
    public  String runEncrypt(String text, String key){
        ...
        String pt;
        int[] ct = new int[8];
        char[] alphs = text.toCharArray();
        //Start encrypt
        KG.GenerateKeys(key);
        for(int i =0;i<alphs.length;i++)
        {
            pt = Print.addZeroBits(Integer.toString(alphs[i],2));
            System.out.println(pt);
            ...
            ct = enc.encrypt( pt ,KG.getK1(),KG.getK2());
            resultBuffer.append(Print.convertFromBitToChar(ct));
        }

        return resultBuffer.toString();
    }

    public String runDecrypt(String text,String key){
...
class Print
{
    static char convertFromBitToChar(int [] pt) {
        String s = "";
        for(int i =0;i< 8;i++){
            s += String.valueOf(pt[i]);
        }
    ...
    static String convertFromBit(int [] pt) {
        String s = "";
        for(int i =0;i< 8;i++){
            s += String.valueOf(pt[i]);
        }
    ...
    static String addZeroBits(String val){
        int off = 0;
        if(val.length() < 8){
            off = 8 - val.length();
            for(int i = 0;i< off;i++){
